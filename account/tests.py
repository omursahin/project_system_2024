...
class UserTest(APITestCase):
    ...
    def setUp(self):
        (MyUser.objects.
         create_superuser(email=self.admin_user["email"],
                          password=self.admin_user["password"],
                          identification_number=self.admin_user["identification_number"]))
        (MyUser.objects.
         create_user(email=self.student_user["email"],
    ...
    def test_create_user_database(self):
        user = MyUser.objects.create_user(email="test_user@test.com", password="test1234",
